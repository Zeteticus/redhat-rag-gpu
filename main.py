...
class GPUVectorStore:
    ...
    def _save_vectors(self, vectors: np.ndarray):
        """Save vectors to file with error handling and atomic writes"""
        try:
            # Save to temporary file first
            temp_file = str(self.vectors_file) + ".tmp"
            np.save(temp_file, vectors)
            
            # Then rename to actual file (more atomic)
            shutil.move(temp_file, str(self.vectors_file))
